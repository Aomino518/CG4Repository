...
void Entity3D::Init(Entity3DCommon* entity3DCommon, const std::string& directoryPath, const std::string& filename)
{
...
void Entity3D::Draw()
{
	...
	// wvp用のCBufferの場所を設定
	cmdList_->SetGraphicsRootConstantBufferView(1, wvpResource_->GetGPUVirtualAddress());
...
MaterialData Entity3D::LoadMaterialTemplateFile(const std::string& directoryPath, const std::string& filename)
{
	// 必要な変数宣言とファイルを開く
...
void Entity3D::LoadObjFile(const std::string& directoryPath, const std::string& filename)
{
...
void Entity3D::CreateBufferResources()
{
	// 頂点リソース
...
void Entity3D::ModelResourcesSetting()
{
	...
	wvpResource_ = CreateBufferResource(Graphics::GetDevice(), sizeof(TransformationMatrix));
	// 書き込むためのアドレスを取得
